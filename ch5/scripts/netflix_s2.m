...
%% ****************************  loading data  ****************************
EstimOpt.DataFile = ('netflix_s2.mat'); % specify the name of the data file
...
DATA.Xa = [DATA.alt==3, DATA.x3, DATA.x4, DATA.x5, (DATA.x6/10)]; 

% DATA.Xa = [DATA.x3, DATA.x4, DATA.x5, (DATA.x6/10).*(DATA.alt<3)]; 
EstimOpt.NamesA = {'ASC3';'AgeGender';'Location';'ExternalData'; 'SearchTime'};
...
% DATA.filter = ones(size(DATA.treatment)) == 1; % modify to include only some observations, e.g. model for male respondents only
INPUT.Y = DATA.Y; 
INPUT.Xa = DATA.Xa;
INPUT.Xm = DATA.Xm;
...
EstimOpt.WTP_space = 0; % number of monetary attributes for WTP-space estimation (need to come last in Xa)
...
% EstimOpt.FullCov = 0;
EstimOpt.Dist = [0;0;0;0;1]; % optional: AdsTime lognormal

% B_backup = [-2.79907114487808,
% -1.41908651026310,
% -2.24979173675302,
% -2.84423140264558,
% -8.91608328388209,
...
%% ****************************     LC     ****************************
EstimOpt.NClass = 3; % default
EstimOpt.ClassScores = 1; % default (yes)
% EstimOpt.BActiveClass = ones(EstimOpt.NVarA,1); % default Specify which coefficients should be kept constant (=0)
